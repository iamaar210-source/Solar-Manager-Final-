...
def stock():
    ...
    ctx = common_ctx(user)
    search = request.args.get("q", "")
    rows = db.get_inventory(search)
    return render_template("stock.html", items=rows, search=search, **ctx)
...
def invoices_add():
    ...
    # Parse items from form (item_name[], qty[], price[])
    names = request.form.getlist("item_name[]")
    qtys = request.form.getlist("qty[]")
    prices = request.form.getlist("price[]")
    items = []
    for n, q, p in zip(names, qtys, prices):
        if n.strip() and float(q or 0) > 0:
            qty, price = float(q), float(p or 0)
            items.append({"item_name": n.strip(), "quantity": qty, "unit": "pcs",
