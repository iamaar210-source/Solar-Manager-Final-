...
def stock():
    try:
        ...
        ctx = common_ctx(user)
        search = request.args.get("q", "")
        rows = db.get_inventory(search) or []
        # Ensure every item is a dict (defensive)
        safe_rows = []
        for r in rows:
            if isinstance(r, dict):
                ...
                try:
                    safe_rows.append(dict(r))
                except Exception:
                    pass
        return render_template("stock.html", items=safe_rows, search=search, **ctx)
    except Exception as e:
        import traceback
        print("STOCK ERROR:", traceback.format_exc())
        flash(f"Stock error: {e}", "error")
        return redirect(url_for("main.dashboard"))
