...
class Database:
    ...
    def get_inventory(self, search=""):
        ...
        try:
            if search:
                cur.execute(
                    "SELECT id, item_name, category, quantity, unit, min_stock, purchase_price, sale_price, notes, updated_at "
                    "FROM inventory WHERE item_name LIKE ? OR category LIKE ? ORDER BY item_name",
                    (f"%{search}%", f"%{search}%")
                )
            else:
                cur.execute(
            ...
            for row in cur.fetchall():
                # Always build a plain dict from column names
                if isinstance(row, dict):
                    d = dict(row)
                else:
                    d = dict(zip(cols, row))
    ...
    def get_invoice(self, iid, location):
        ...
        inv["items"] = _rows_to_dicts(cur.fetchall())
        conn.close()
        return inv
    ...
    def get_quotation(self, qid, location):
        ...
        q["items"] = _rows_to_dicts(cur.fetchall())
        conn.close()
        return q
