...
class Database:
    ...
    def update_customer(self, cid, location, name=None, phone=None, email=None, address=None, project_status=None, notes=None):
        conn = self._conn(); cur = conn.cursor()
        cur.execute("SELECT * FROM customers WHERE id=? AND location=?", (cid, location))
        row = cur.fetchone()
        if not row:
            conn.close(); raise ValueError("Customer not found")
        r = dict(row) if not isinstance(row, dict) else row
        cur.execute(
            """UPDATE customers SET name=?, phone=?, email=?, address=?, project_status=?, notes=?, updated_at=?
               WHERE id=? AND location=?""",
            (name if name is not None else r.get("name"),
             phone if phone is not None else r.get("phone"),
             email if email is not None else r.get("email"),
             address if address is not None else r.get("address"),
             project_status if project_status is not None else r.get("project_status"),
             notes if notes is not None else r.get("notes"),
             datetime.now().isoformat(), cid, location)
        )
        conn.commit(); conn.close()

    def get_customer(self, cid, location):
        conn = self._conn(); cur = conn.cursor()
        cur.execute("SELECT * FROM customers WHERE id=? AND location=?", (cid, location))
    ...
    def get_expense(self, eid, location):
        conn = self._conn(); cur = conn.cursor()
        cur.execute("SELECT * FROM expenses WHERE id=? AND location=?", (eid, location))
        row = cur.fetchone(); conn.close()
        return _row_to_dict(row) if row else None

    def update_due(self, did, location, customer_name=None, total_amount=None, paid_amount=None, due_date=None, status=None, notes=None):
        conn = self._conn(); cur = conn.cursor()
        cur.execute("SELECT * FROM dues WHERE id=? AND location=?", (did, location))
        row = cur.fetchone()
        if not row:
            conn.close(); raise ValueError("Due not found")
        r = dict(row) if not isinstance(row, dict) else row
        total = float(total_amount if total_amount is not None else r.get("total_amount") or 0)
        paid = float(paid_amount if paid_amount is not None else r.get("paid_amount") or 0)
        if status is None:
            status = "Paid" if paid >= total and total > 0 else ("Partial" if paid > 0 else "Pending")
        cur.execute(
            """UPDATE dues SET customer_name=?, total_amount=?, paid_amount=?, due_date=?, status=?, notes=?
               WHERE id=? AND location=?""",
            (customer_name if customer_name is not None else r.get("customer_name"),
             total, paid, due_date if due_date is not None else r.get("due_date"),
             status, notes if notes is not None else r.get("notes"), did, location)
        )
        conn.commit(); conn.close()

    def get_due(self, did, location):
        conn = self._conn(); cur = conn.cursor()
        cur.execute("SELECT * FROM dues WHERE id=? AND location=?", (did, location))
        row = cur.fetchone(); conn.close()
        return _row_to_dict(row) if row else None

    def update_employee(self, eid, location, name=None, phone=None, role=None, monthly_salary=None, status=None, notes=None):
        conn = self._conn(); cur = conn.cursor()
        cur.execute("SELECT * FROM employees WHERE id=? AND location=?", (eid, location))
        row = cur.fetchone()
        if not row:
            conn.close(); raise ValueError("Employee not found")
        r = dict(row) if not isinstance(row, dict) else row
        cur.execute(
            """UPDATE employees SET name=?, phone=?, role=?, monthly_salary=?, status=?, notes=?, updated_at=?
               WHERE id=? AND location=?""",
            (name if name is not None else r.get("name"),
             phone if phone is not None else r.get("phone"),
             role if role is not None else r.get("role"),
             float(monthly_salary if monthly_salary is not None else r.get("monthly_salary") or 0),
             status if status is not None else r.get("status"),
             notes if notes is not None else r.get("notes"),
             datetime.now().isoformat(), eid, location)
        )
        conn.commit(); conn.close()

    def get_employee(self, eid, location):
        conn = self._conn(); cur = conn.cursor()
        cur.execute("SELECT * FROM employees WHERE id=? AND location=?", (eid, location))
        row = cur.fetchone(); conn.close()
        return _row_to_dict(row) if row else None

    def update_visit(self, vid, location, customer_name=None, phone=None, visit_date=None, notes=None, reminder_date=None):
        conn = self._conn(); cur = conn.cursor()
        cur.execute("SELECT * FROM customer_visits WHERE id=? AND location=?", (vid, location))
        row = cur.fetchone()
        if not row:
            conn.close(); raise ValueError("Visit not found")
        r = dict(row) if not isinstance(row, dict) else row
        cur.execute(
            """UPDATE customer_visits SET customer_name=?, phone=?, visit_date=?, notes=?, reminder_date=?
               WHERE id=? AND location=?""",
            (customer_name if customer_name is not None else r.get("customer_name"),
             phone if phone is not None else r.get("phone"),
             visit_date if visit_date is not None else r.get("visit_date"),
             notes if notes is not None else r.get("notes"),
             reminder_date if reminder_date is not None else r.get("reminder_date"),
             vid, location)
        )
        conn.commit(); conn.close()

    def get_visit(self, vid, location):
        conn = self._conn(); cur = conn.cursor()
        cur.execute("SELECT * FROM customer_visits WHERE id=? AND location=?", (vid, location))
        row = cur.fetchone(); conn.close()
        return _row_to_dict(row) if row else None

    def update_quotation(self, qid, location, customer_name=None, customer_phone=None, customer_address=None,
                         discount=None, notes=None, date=None, status=None):
        conn = self._conn(); cur = conn.cursor()
        cur.execute("SELECT * FROM quotations WHERE id=? AND location=?", (qid, location))
        row = cur.fetchone()
        if not row:
            conn.close(); raise ValueError("Quotation not found")
        r = dict(row) if not isinstance(row, dict) else row
        subtotal = float(r.get("subtotal") or 0)
        discount = float(discount if discount is not None else r.get("discount") or 0)
        total = max(0, subtotal - discount)
        cur.execute(
            """UPDATE quotations SET customer_name=?, customer_phone=?, customer_address=?,
               discount=?, total=?, notes=?, date=?, status=? WHERE id=? AND location=?""",
            (customer_name if customer_name is not None else r.get("customer_name"),
             customer_phone if customer_phone is not None else r.get("customer_phone"),
             customer_address if customer_address is not None else r.get("customer_address"),
             discount, total,
             notes if notes is not None else r.get("notes"),
             date if date is not None else r.get("date"),
             status if status is not None else r.get("status"),
             qid, location)
        )
        conn.commit(); conn.close()
    ...
    def get_inventory_item(self, iid):
        conn = self._conn(); cur = conn.cursor()
        cur.execute("SELECT * FROM inventory WHERE id=?", (iid,))
        row = cur.fetchone(); conn.close()
        return _row_to_dict(row) if row else None
    ...
    def update_invoice(self, iid, location, customer_name=None, customer_phone=None,
                       customer_address=None, discount=None, amount_paid=None, notes=None, date=None):
        conn = self._conn()
        cur = conn.cursor()
        cur.execute("SELECT * FROM invoices WHERE id=? AND location=?", (iid, location))
        row = cur.fetchone()
        ...
        inv = dict(row) if not isinstance(row, dict) else row
        subtotal = float(inv.get("subtotal") or 0)
        discount = float(discount if discount is not None else inv.get("discount") or 0)
        total = max(0, subtotal - discount)
        amount_paid = float(amount_paid if amount_paid is not None else inv.get("amount_paid") or 0)
        ...
        cur.execute(
            """UPDATE invoices SET customer_name=?, customer_phone=?, customer_address=?,
               discount=?, total=?, amount_paid=?, payment_status=?, notes=?, date=?
               WHERE id=? AND location=?""",
            (
                customer_name if customer_name is not None else inv.get("customer_name"),
                customer_phone if customer_phone is not None else inv.get("customer_phone"),
                customer_address if customer_address is not None else inv.get("customer_address"),
                discount, total, amount_paid, status,
                notes if notes is not None else inv.get("notes"),
                date if date is not None else inv.get("date"),
                iid, location,
            ),
        )
        conn.commit()
    ...
    def add_vendor_order(self, location, vendor_name, item_name, quantity, unit="pcs", unit_price=0,
                         sale_price=0, notes="", vendor_id=None):
        """Place order, auto-receive into shared stock with purchase + sale prices."""
        conn = self._conn()
        cur = conn.cursor()
        now = datetime.now().isoformat()
        unit_price = float(unit_price or 0)
        sale_price = float(sale_price or 0) or (unit_price * 1.2 if unit_price else 0)
        ...
        oid = cur.lastrowid
        # Auto-receive into shared stock
        ...
        if existing:
            ...
            cur.execute(
                "UPDATE inventory SET quantity=quantity+?, purchase_price=?, sale_price=?, updated_at=? WHERE id=?",
                (quantity, unit_price, sale_price, now, eid),
            )
        else:
            cur.execute(
                """INSERT INTO inventory (item_name, category, quantity, unit, min_stock, purchase_price, sale_price, updated_at)
                   VALUES (?,?,?,?,?,?,?,?)""",
                (item_name, "General", quantity, unit, 5, unit_price, sale_price, now),
            )
        conn.commit()
        conn.close()
        return oid
    ...
    def get_purchase_price(self, item_name):
        conn = self._conn()
        cur = conn.cursor()
        try:
            cur.execute("SELECT purchase_price FROM inventory WHERE item_name=?", (item_name,))
            row = cur.fetchone()
            if not row:
                return 0.0
            return float(row["purchase_price"] if not isinstance(row, tuple) else (row[0] or 0))
        finally:
            conn.close()
